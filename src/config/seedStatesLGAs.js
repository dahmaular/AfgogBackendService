...
const { LGA } = require("../api/models/LGA");
...
async function seedStatesAndLGAs() {
  try {
    // Connect to MongoDB
    const mongoUri = process.env.MONGODB_URI || config.get("db");
    ...
    console.log("Cleared existing states and LGAs...");
    ...
    for (const stateData of nigeriaData) {
      ...
      console.log(`Created ${stateData.lgas.length} LGAs for ${state.name}`);
    }

    console.log("\n✅ Successfully seeded all Nigerian states and LGAs!");
    
    // Display summary
    const stateCount = await State.countDocuments();
    const lgaCount = await LGA.countDocuments();
    console.log(`\nTotal States: ${stateCount}`);
    console.log(`Total LGAs: ${lgaCount}`);
